#!/usr/bin/env python3
"""FastAPI server for App Runner and local dev — wraps answer/answer.py.

Exposes GET /answer → calls answer.handler → returns NCCO.

App Runner runs this in production. For local dev, run with ngrok and set Vonage
...
async def vonage_answer(request: Request) -> JSONResponse:
    """Vonage GET /answer webhook — wraps answer.handler."""
